...
const CourseReader = () => {
  ...
  const loadPage = async (page) => {
    ...
    setPageContent('');
    try {
      const { data } = await api.post('/ai/course-page', {
        courseId,
        courseTitle,
        page,
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-4xl mx-auto py-6 px-4">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <button onClick={() => navigate('/courses')}
                className="text-indigo-600 text-sm font-semibold mb-1">
                ← Back to Courses
              </button>
              ...
              <div className="text-sm text-gray-700 leading-relaxed space-y-3">
                {pageContent.split('\n').map((line, i) => {
                  if (line.startsWith('## ')) return (
                    <h3 key={i} className="text-lg font-black text-indigo-700 border-b border-indigo-100 pb-2 mt-4">
                      {line.replace('## ', '')}
                    </h3>
                  );
                  if (line.startsWith('### ')) return (
                    <h4 key={i} className="text-base font-bold text-gray-800 mt-3">
                      {line.replace('### ', '')}
                    </h4>
                  );
                  ...
                  if (line.includes('=') || line.includes('(') || line.includes('{')) return (
                    <div key={i} className="bg-gray-900 text-green-400 px-4 py-2 rounded-lg font-mono text-xs">
                      {line}
                    </div>
                  );
        ...
        {/* Certificate button if all done */}
        {allCompleted && (
